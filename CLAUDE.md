...
- **Current Access**: shows a project badge + days-left + small "Revoke" link per grant, with a row of one-click duration chips (24h / 3d / 7d / 30d / 90d / Infinity — Infinity stores `expiresAt = NEVER_EXPIRES_AT` (year 9999) so all access gates work unchanged; displays check `isNeverExpiring()`; grants only, never share links, see `lib/durations.ts`) that POST to `/portal/admin/extend` — each click both extends the grant and sends the user a Resend `grant-confirmation` notification.
- **Grant Access** (formerly "Direct Grant" + "Email Grant" columns): single flow — pick projects + duration, click Grant Now. Grant is instant AND a confirmation email is queued via Resend to the target user's Clerk email (reply-to set to admin). No separate "Send Email" button anymore; the legacy `/portal/admin/grant` chip-based-invite route still exists for the user-initiated `/portal/request-access` flow. **Checkboxes pre-check every project the user already has access to** (matched by slug against live grants) so the form mirrors current state; submitting refreshes every checked grant to the selected duration. Unchecked existing grants are left alone — use the per-grant Revoke link to remove them. Page is `force-dynamic` so re-renders show grants made elsewhere (e.g. via accept email) immediately.
